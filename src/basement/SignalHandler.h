...
public:
    explicit SignalHandler(const std::initializer_list<int> &signals)
        ...
        for (int signum : signals)
            if (sigaddset(&this->set_, signum) != 0)
                throw SignalHandlerException("sigaddset: invalid signal " + std::to_string(signum));

        int s = pthread_sigmask(SIG_BLOCK, &this->set_, nullptr);
        if (s != 0)
            throw SignalHandlerException(std::string("pthread_sigmask: ") + std::strerror(s));
    }
    ...
    int sigwait() const {
        ...
        if (ret != 0)
            throw SignalHandlerException(std::string("sigwait: ") + std::strerror(ret));
        return signum;
    }
    ...
    auto async_sigwait_handler(std::function<bool(int)> handler) const {
        return std::async(std::launch::async, &SignalHandler::sigwait_handler, this, std::move(handler));
    }
